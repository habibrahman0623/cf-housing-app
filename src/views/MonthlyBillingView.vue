<template>
  <div class="w-full space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
      <div @click="showMonthlyModal = true" class="bg-white p-6 rounded-2xl shadow-sm border-b-4 border-emerald-500 cursor-pointer hover:shadow-md transition-all">
        <div class="text-3xl mb-2">📅</div>
        <h4 class="font-bold text-slate-800">Generate Monthly Bill</h4>
        <p class="text-xs text-gray-500 font-medium">সবার জন্য মাসিক বিল তৈরি করুন</p>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead class="bg-slate-50 text-[10px] uppercase text-slate-500 font-bold tracking-widest">
            <tr>
              <th class="px-6 py-4 border-b">Billing Period</th>
          ...
          <tbody class="divide-y divide-slate-100 text-sm font-medium">
            <tr v-for="i in 5" :key="i" class="hover:bg-slate-50/50 transition-colors">
              <td class="px-6 py-4 text-slate-700 font-bold">April 2026</td>
              <td class="px-6 py-4 text-slate-600">৳ 200.00</td>
              <td class="px-6 py-4 text-slate-600">৳ 24,000 (120 shares)</td>
    ...
    <Teleport to="body">
      <div v-if="showMonthlyModal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" @click="showMonthlyModal = false"></div>
        ...
        <div @click.stop class="relative bg-white rounded-3xl shadow-2xl w-full max-w-md z-10 border border-emerald-100 flex flex-col max-h-[95vh]">
          ...
          <div class="p-6 pb-2 text-center flex-shrink-0">
            ...
            <p class="text-xs text-gray-500">মাসিক কন্ট্রিবিউশন বিল তৈরি করুন</p>
          </div>
          
          <form @submit.prevent="submitMonthlyBills" class="p-6 pt-2 space-y-4 overflow-y-auto">
            <div>
              <label class="block text-[10px] font-bold text-gray-400 uppercase ml-1 mb-1">Select Month</label>
              <input v-model="monthlyBillData.selected_date" type="date" required class="w-full px-4 py-3 border-2 border-gray-100 rounded-xl outline-none focus:border-emerald-500 transition-all font-semibold text-sm">
              <div v-if="monthlyBillData.selected_date" class="mt-2 flex items-center gap-2 bg-blue-50 p-2 rounded-lg border border-blue-100 w-fit">
                <span class="text-[11px] text-blue-600 font-bold uppercase tracking-tight">📍 Selected: {{ new Date(monthlyBillData.selected_date).toLocaleString('default', { month: 'long', year: 'numeric' }) }}</span>
              </div>
            </div>

            <div>
              <label class="block text-[10px] font-bold text-gray-400 uppercase ml-1 mb-1">Rate Per Share (৳)</label>
              <div class="relative">
                <span class="absolute left-4 top-3 text-gray-400 font-bold">৳</span>
                <input v-model.number="monthlyBillData.rate_per_share" type="number" required class="w-full pl-10 pr-4 py-3 border-2 border-gray-100 rounded-xl outline-none focus:border-emerald-500 transition-all text-slate-700 font-bold text-sm">
              </div>
            </div>

            <div>
              <label class="block text-[10px] font-bold text-gray-400 uppercase ml-1 mb-1">Payment Due Date</label>
              <input v-model="monthlyBillData.due_date" type="date" required class="w-full px-4 py-3 border-2 border-gray-100 rounded-xl outline-none focus:border-emerald-500 transition-all font-semibold text-sm">
            </div>
            
            <div class="flex gap-3 pt-4 sticky bottom-0 bg-white">
              <button type="button" @click="showMonthlyModal = false" class="flex-1 py-3 text-sm text-gray-500 font-bold hover:bg-gray-50 rounded-xl transition-all border border-gray-100">
                বাতিল
              </button>
    ...
    <Teleport to="body">
      <div v-if="showFineModal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        ...
        <div @click.stop class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm z-10 border border-orange-100 p-6">
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Billing Period</label>
              <input v-model="fineData.billing_period" type="date" required class="w-full px-4 py-3 border-2 border-gray-100 rounded-xl outline-none focus:border-emerald-500 transition-all font-semibold text-sm">
            </div>
      ...
      <div v-if="showWaiveModal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        ...
        <div @click.stop class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm z-10 border border-blue-100 p-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Billing Period</label>
              <input v-model="waiveData.billing_period" type="date" required class="w-full px-4 py-3 border-2 border-gray-100 rounded-xl outline-none focus:border-emerald-500 transition-all font-semibold text-sm">
            </div>
      ...
      <div v-if="showCancelModal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        ...
        <div @click.stop class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm z-10 border border-red-100 p-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Billing Period</label>
              <input v-model="cancelData.billing_period" type="date" required class="w-full px-4 py-3 border-2 border-gray-100 rounded-xl outline-none focus:border-emerald-500 transition-all font-semibold text-sm">
            </div>
...
const fineData = ref({
  billing_period: '', // এটি আমরা watch দিয়ে অটো সেট করব
...
const waiveData = ref({
  member_code: '',
  billing_period: '', // এটিও আমরা অটো-সিঙ্ক করব
  reason: ''
});
...
const submitMonthlyBills = async () => {
  if (!confirm("আপনি কি নিশ্চিত?")) return;
  isGenerating.value = true;
  try {
    //const token = localStorage.getItem('token');
    const payload = {
      billing_period: monthlyBillData.value.billing_period,
      rate_per_share: monthlyBillData.value.rate_per_share,
      due_date: new Date(monthlyBillData.value.due_date).toISOString() 
    };
    await api.post('/billing/generate-monthly-contribution', 
      payload
    );
...
const applyBulkFine = async () => {
  if (!confirm(`আপনি কি নিশ্চিত যে ${fineData.value.billing_period} মাসের জন্য ৳${fineData.value.fine_amount} জরিমানা যোগ করবেন?`)) return;
  ...
  try {
    const response = await api.post('/billing/apply-bulk-fine-monthly', {
      billing_period: fineData.value.billing_period,
...
const waiveFine = async () => {
  ...
  try {
    const response = await api.post('/billing/waive-fine-monthly', {
      member_code: waiveData.value.member_code,
      billing_period: waiveData.value.billing_period,
...
const cancelBill = async () => {
  if (!cancelData.value.member_code || !cancelData.value.reason || !cancelData.value.billing_period) {
  ...
  try {
    ...
    const response = await api.delete('/billing/cancel-monthly-bill', {
      data: {
        member_code: cancelData.value.member_code,
        billing_period: cancelData.value.billing_period,
    ...
    cancelData.value.billing_period = '';
